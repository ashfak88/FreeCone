...
export default function UserManagementPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (activeRoleTab !== "all") queryParams.append("role", activeRoleTab);
  ...
  }, [currentUser, activeRoleTab, activeStatusFilter, search]);
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 mb-10 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="relative flex-1 w-full">
          ...
          <input 
            ...
            className="w-full bg-slate-50 dark:bg-slate-950 border-none rounded-2xl py-3.5 pl-12 pr-4 text-sm font-bold placeholder:text-slate-500 focus:ring-2 focus:ring-primary/20 transition-all"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        
        <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 p-1.5 rounded-2xl">
          {["all", "freelancer", "client"].map((role) => (
            <button
              key={role}
            ...
            >
              {role === 'all' ? 'Users' : role === 'freelancer' ? 'Talents' : 'Clients'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-8 px-2 overflow-x-auto pb-4 scrollbar-hide">
        {["all", "active", "pending", "blocked"].map((status) => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 white-nowrap rounded-full text-xs font-black uppercase tracking-widest transition-all ${
            ...
            {users.map((u, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-6">
                  <div className="size-16 rounded-2xl overflow-hidden border-2 border-slate-100 dark:border-slate-800 p-0.5 group-hover:border-primary/30 transition-colors">
                    <img 
                ...
                <div className="flex items-center gap-2 pt-4 border-t border-slate-50 dark:border-slate-800/50">
                  <button 
                    onClick={() => router.push(`/talent/${u._id}`)}
                    className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] font-black uppercase tracking-[0.2em] py-3 rounded-xl hover:bg-primary dark:hover:bg-primary dark:hover:text-white transition-all shadow-lg shadow-black/10"
                  >
                    Details
                  </button>
                  ...
                  {u.status === 'blocked' ? (
                    <button 
                      ...
                      className="p-3 bg-green-50 dark:bg-green-900/20 text-green-600 rounded-xl hover:bg-green-600 hover:text-white transition-all"
                    >
                    ...
                    <button 
                      ...
                      className="p-3 bg-red-50 dark:bg-red-900/20 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all"
                    >
